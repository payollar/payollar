...
export function FeaturedCarousel() {
  ...
  const prevSlide = () => {
    setCurrentSlide((prev) => (prev - 1 + featuredTalents.length) % featuredTalents.length)
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {isMobile ? (
          <div className="relative">
                    ...
                    <Card className="overflow-hidden bg-white/5 backdrop-blur-sm border-white/10">
                      <div className="relative">
                        ...
                        <div className="absolute inset-0 flex items-center justify-center">
                          <Button
                            size="sm"
                            className="bg-white/20 backdrop-blur-sm hover:bg-white/30 text-white rounded-full p-3"
                          >
                      ...
                      <CardContent className="p-4 space-y-3">
                        ...
                        {/* Description */}
                        <p className="text-gray-300 text-sm leading-relaxed line-clamp-2">{talent.description}</p>
                        ...
                        <div className="flex flex-wrap gap-1">
                          {talent.genres.slice(0, 2).map((genre, index) => (
                            <Badge
                              key={index}
                              className="bg-purple-600/30 text-purple-200 border-purple-400/30 text-xs"
                            >
                              {genre}
                            </Badge>
                          ))}
            ...
            <Button
              onClick={prevSlide}
              className="absolute left-2 top-1/2 transform -translate-y-1/2 bg-white/10 backdrop-blur-sm hover:bg-white/20 text-white rounded-full p-2"
            >
              <ChevronLeft className="w-5 h-5" />
            </Button>
            <Button
              onClick={nextSlide}
              className="absolute right-2 top-1/2 transform -translate-y-1/2 bg-white/10 backdrop-blur-sm hover:bg-white/20 text-white rounded-full p-2"
            >
          ...
          <div className="relative">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 min-h-[500px]">
                  ...
                  <div className="relative overflow-hidden">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <Button
                        size="lg"
                        className="bg-white/20 backdrop-blur-sm hover:bg-white/30 text-white rounded-full p-4 transition-all duration-300 transform hover:scale-110"
                      >
                    ...
                    {/* Floating music note */}
                    <div className="absolute top-6 right-6 p-3 bg-white/10 backdrop-blur-sm rounded-full animate-pulse">
                    ...
                    <div className="absolute top-6 left-6 flex flex-col space-y-2">
                      ...
                      <Badge
                        className={`${
                          currentTalent.availability === "Available"
                            ? "bg-green-500 text-white"
                            : "bg-yellow-500 text-white"
                        }`}
                      >
                  ...
                  <div className="p-8 lg:p-12 flex flex-col justify-center space-y-6">
                    ...
                    {/* Name and title */}
                    <div>
                      <h3 className="text-3xl font-bold text-white mb-2">{currentTalent.name}</h3>
                    ...
                    {/* Description */}
                    <p className="text-gray-300 leading-relaxed">{currentTalent.description}</p>

                    {/* Achievements */}
                    <div>
                      <h4 className="text-white font-semibold mb-3">Key Achievements</h4>
                      <div className="space-y-2">
                        {currentTalent.achievements.map((achievement, index) => (
                    ...
                    <div className="flex flex-wrap gap-2">
                      {currentTalent.genres.map((genre, index) => (
                        <Badge key={index} className="bg-purple-600/30 text-purple-200 border-purple-400/30">
                          {genre}
                        </Badge>
                      ))}
                    </div>
                    ...
                    <div className="flex items-center justify-between pt-4">
                      ...
                      <div className="flex space-x-3">
                        ...
                        <Button className="bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white px-6 py-3 rounded-full font-semibold transition-all duration-300 transform hover:scale-105">
                          Book Now
                        </Button>
                      </div>
                    </div>
            ...
            <Button
              onClick={prevSlide}
              className="absolute left-4 top-1/2 transform -translate-y-1/2 bg-white/10 backdrop-blur-sm hover:bg-white/20 text-white rounded-full p-3 transition-all duration-300"
            >
              <ChevronLeft className="w-6 h-6" />
            </Button>
            <Button
              onClick={nextSlide}
              className="absolute right-4 top-1/2 transform -translate-y-1/2 bg-white/10 backdrop-blur-sm hover:bg-white/20 text-white rounded-full p-3 transition-all duration-300"
            >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8 mt-8 md:mt-16 text-center">
          <div>
            ...
            <div className="text-gray-400 text-sm md:text-base">Featured Talents</div>
          </div>
          <div>
          ...
          <div>
            ...
            <div className="text-gray-400 text-sm md:text-base">Total Reach</div>
          </div>
          <div>
